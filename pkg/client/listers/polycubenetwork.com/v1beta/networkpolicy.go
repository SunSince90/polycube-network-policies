...
type NetworkPolicyLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta.NetworkPolicy, err error)
...
// List lists all NetworkPolicies in the indexer.
func (s *networkPolicyLister) List(selector labels.Selector) (ret []*v1beta.NetworkPolicy, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta.NetworkPolicy))
	})
	return ret, err
}
...
type NetworkPolicyNamespaceLister interface {
	...
	Get(name string) (*v1beta.NetworkPolicy, error)
...
// List lists all NetworkPolicies in the indexer for a given namespace.
func (s networkPolicyNamespaceLister) List(selector labels.Selector) (ret []*v1beta.NetworkPolicy, err error) {
...
// Get retrieves the NetworkPolicy from the indexer for a given namespace and name.
func (s networkPolicyNamespaceLister) Get(name string) (*v1beta.NetworkPolicy, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta.Resource("networkpolicy"), name)
	}
	return obj.(*v1beta.NetworkPolicy), nil
}
